...
# cal_vel v.3.4 ->agular의 오른 왼 회전 인식하는거 추가필요
def calculate_velocity_commands(target_tvec, turtlebot_tvec, turtlebot_rvec, target_rvec, linear_gain=0.1, angular_gain=0.1):
    # 타겟 마커의 방향(z축)과 터틀봇의 전진방향(y축) 계산
    turtlebot_rotation, _ = cv2.Rodrigues(turtlebot_rvec)
    target_rotation, _ = cv2.Rodrigues(target_rvec)
    
    # 터틀봇의 전진 방향 (y축)
    turtlebot_forward = turtlebot_rotation[:, 1]
    # 타겟의 normal 방향 (z축)
    target_normal = -target_rotation[:, 2]  # 마이너스는 마커가 앞면이 보이는 방향
